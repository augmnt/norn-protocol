...
Names work seamlessly in transfers -- pass a NornName instead of a hex address. See the [NornNames documentation](/docs/names) for details on name records, reverse resolution, and transfer mechanics.
...
The `rewards` command shows the current epoch fees being accumulated, blocks remaining until the next distribution, and a per-validator breakdown of projected rewards based on stake weight.
