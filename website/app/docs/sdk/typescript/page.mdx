...
// Get thread state (balances, names, etc.)
...
// Register a name (requires name, owner hex, and signed knot hex)
...
// Build a transfer (NORN uses 12 decimals by default)
const transferHex = buildTransfer(wallet, {
  to: "0xrecipient...",
  amount: parseAmount("10.5"),
});
await client.submitKnot(transferHex);

// Register a NornName
const nameHex = buildNameRegistration(wallet, "alice");
const ownerHex = "0x" + Buffer.from(wallet.address).toString("hex");
await client.registerName("alice", ownerHex, nameHex);
