...
function ProposalCard({ proposal }: { proposal: GovProposal }) {
  ...
  return (
    <Link href={`/apps/governance/${proposal.id.toString()}`}>
...
export default function GovernanceDashboardPage() {
  ...
  if (!GOVERNANCE_LOOM_ID) {
    return (
      <PageContainer title="DAO Governance">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <AlertCircle className="h-4 w-4" />
              Governance contract not configured. Set{" "}
              <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">
                NEXT_PUBLIC_GOVERNANCE_LOOM_ID
              </code>{" "}
              in your environment.
            </div>
          </CardContent>
  ...
  // Not yet initialized
  if (!fetching && !config && GOVERNANCE_LOOM_ID) {
    return (
      <PageContainer
    ...
    <PageContainer
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          <Link href="/apps/governance/create">
            <Button size="sm">
