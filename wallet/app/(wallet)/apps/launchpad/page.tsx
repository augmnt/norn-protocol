...
export default function LaunchpadDashboardPage() {
  ...
  const { getConfig, getTotalRaised, getContribution, finalize, loading } =
    useLaunchpad(LAUNCHPAD_LOOM_ID);
  ...
  }, [getConfig, getTotalRaised, getContribution, activeAddress]);
  ...
  if (!LAUNCHPAD_LOOM_ID) {
    return (
      <PageContainer title="Token Launchpad">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <AlertCircle className="h-4 w-4" />
              Launchpad contract not configured. Set{" "}
              <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">
                NEXT_PUBLIC_LAUNCHPAD_LOOM_ID
              </code>{" "}
              in your environment.
            </div>
          </CardContent>
  ...
  // Not yet initialized
  if (!fetching && !config && LAUNCHPAD_LOOM_ID) {
    return (
      <PageContainer
    ...
    <PageContainer
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          {isActive && (
            <Link href="/apps/launchpad/contribute">
              <Button size="sm">
