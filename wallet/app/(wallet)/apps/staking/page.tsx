...
export default function StakingDashboardPage() {
  ...
  if (!STAKING_LOOM_ID) {
    return (
      <PageContainer title="Staking Vault">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <AlertCircle className="h-4 w-4" />
              Staking contract not configured. Set{" "}
              <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">
                NEXT_PUBLIC_STAKING_LOOM_ID
              </code>{" "}
              in your environment.
            </div>
          </CardContent>
  ...
  // Not yet initialized
  if (!fetching && !config && STAKING_LOOM_ID) {
    return (
      <PageContainer
        title="Staking Vault"
        action={
      ...
      >
        <InitializeForm loomId={STAKING_LOOM_ID} onSuccess={fetchData} />
      </PageContainer>
    );
  }
    ...
    <PageContainer
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          <Link href="/apps/staking/stake">
            <Button size="sm">
              <Plus className="mr-1.5 h-3.5 w-3.5" />
              Stake
            </Button>
          </Link>
          <Link href="/apps/staking/fund">
