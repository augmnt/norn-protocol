...
function OrderCard({ order }: { order: SwapOrder }) {
  return (
    <Link href={`/apps/swap/${order.id.toString()}`}>
...
export default function SwapDashboardPage() {
  ...
  const { getOrder, getOrderCount, loading } = useSwap(SWAP_LOOM_ID);
  const [orders, setOrders] = useState<SwapOrder[]>([]);
  const [fetching, setFetching] = useState(false);

  const fetchOrders = useCallback(async () => {
    if (!SWAP_LOOM_ID) return;
    setFetching(true);
    try {
  ...
  if (!SWAP_LOOM_ID) {
    return (
      <PageContainer title="OTC Swap">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <AlertCircle className="h-4 w-4" />
              Swap contract not configured. Set{" "}
              <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">
                NEXT_PUBLIC_SWAP_LOOM_ID
              </code>{" "}
              in your environment.
            </div>
          </CardContent>
        ...
        <TabsContent value="open" className="space-y-3">
          {fetching || loading ? (
            ...
            openOrders
              .slice()
              .reverse()
              .map((o) => <OrderCard key={o.id.toString()} order={o} />)
          )}
        </TabsContent>
