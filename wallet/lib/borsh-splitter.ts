/**
 * Borsh encoding/decoding for the Splitter contract messages.
 *
...
// Discriminants: Initialize=0, Split=1

export function encodeInitialize(
  name: string,
  recipients: { address: string; shareBps: bigint }[]
): string {
  const recipientParts: Uint8Array[] = [];
  for (const r of recipients) {
    recipientParts.push(hexToBytes(r.address));
    recipientParts.push(encodeU64(r.shareBps));
  }

  const parts = concat(
    new Uint8Array([0]),
    encodeString(name),
    encodeU32LE(recipients.length),
    ...recipientParts
  );
  return bytesToHex(parts);
}

export function encodeSplit(tokenId: string, amount: bigint): string {
  const parts = concat(
    new Uint8Array([1]),
    hexToBytes(tokenId),
    encodeU128(amount)
...
export function decodeSplitterConfig(hex: string): SplitterConfig {
  ...
  [recipientCount, offset] = readU32(data, offset);
  const recipients: Recipient[] = [];
  for (let i = 0; i < recipientCount; i++) {
    let address: string;
    [address, offset] = readAddress(data, offset);
    let shareBps: bigint;
    [shareBps, offset] = readU64(data, offset);
