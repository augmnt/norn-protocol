...
pub async fn run(amount: u128, yes: bool, rpc_url: Option<&str>) -> Result<(), WalletError> {
    ...
    if !yes {
        println!();
        println!("  {}", style_bold().apply_to("Unstake Tokens"));
        print_divider();
        println!(
            "  Amount:    {}",
        ...
        println!(
            "  {}",
            style_dim().apply_to("Tokens will be available after bonding period")
        );
        println!();

        if !confirm("Unstake these tokens?")? {
            println!("  Cancelled.");
            return Ok(());
        }
    }
    ...
    let mut op = StakeOperation::Unstake {
    ...
    match &mut op {
        StakeOperation::Unstake { signature: s, .. } => *s = signature,
        _ => unreachable!(),
    }
    ...
    if result.success {
        print_success("Unstake operation submitted successfully");
    } else {
        return Err(WalletError::Other(
